...
// <copyright file="NewWorkPieceId.cs" company="Breanos GmbH">
// Copyright Notice:
...
namespace CWF.Tasks.NewWorkPieceId
{
  public class NewWorkPieceId : CWF.Core.StatefulActivity<FertigungszelleWorkflowState>
  {
    private static NLog.Logger logger = LogManager.GetCurrentClassLogger();
    public NewWorkPieceId(ActivityMemento activityMememnto) : base(activityMememnto)
    {
    }
    ...
    public override void Run(System.Threading.CancellationToken token, object state, object parameterDto = null)
    {
      if (!token.IsCancellationRequested)
      {
        ...
        StateToken.Workpieceid = rnd.Next();

        Console.WriteLine("WorkpieceCountNumber: " + StateToken.Workpiececount);

        Console.WriteLine("New Workpiece: " + StateToken.Workpieceid);
