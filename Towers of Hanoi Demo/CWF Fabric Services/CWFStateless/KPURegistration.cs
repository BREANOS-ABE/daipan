...
{
    public class KpuStateEventArg : EventArgs
    {
        public KpuStateEventArg(string KpuId, string Commando)
        {
            this.KpuId = KpuId;
    ...
    public class KPURegistration //: INotifyPropertyChanged
    {
        ...
        public static readonly string QueueString = "queue://LineTopic_1";
        ...
        public delegate void KpuStateEventHandler(object sender, KpuStateEventArg e);
        public KpuStateEventHandler PropertyChanged;

        // public event PropertyChangedEventHandler PropertyChanged;

        public KPURegistration(string kpuPath, string tempDir, string endpoint, string user, string password, string activitiesDir, string fsmDir, string workflowsDir, string xsdDir)
        {
            KpuDir = kpuPath;
            TempDir = tempDir;
        ...
        private void KpuMessageHandlingMethod(object sender, OnMessageEventArgs e)
        {
            ...
            {
                WriteManifest(KpuDir, "HanoiLibrary.HanoiWorkflowState");
                GenerateZipFile(KpuDir, TempDir);
            ...
            {                
                if (SerializationHelper.TryUnpack(e.Content, out ExecuteRequest executeRequestDto))
                {
                    ...
                    PropertyChanged?.Invoke(this, new KpuStateEventArg(executeRequestDto.KpuId, executeRequestDto.Action));
                }
            }
            else if ((e.Properties["ContentType"] as string).CompareTo(BrokerCommands.KPU_DEPLOYMENT) == 0)
            {
                logger.Debug($"In KpuMessageHandlingMethod KPU_DEPLOYMENT branch");
                KpuDeployment(e);
                StartActor(e);
            }
        }
        ...
        private void KpuDeployment(OnMessageEventArgs e)
        {
            var isUnpackSuccessful = BreanosConnectors.SerializationHelper.TryUnpack(e.Content, out byte[] zipData);
            string filePath = TempDir + "\\" + e.Properties["KpuId"] + ".zip";

            System.IO.File.WriteAllBytes(filePath, zipData);
            string extension = System.IO.Path.GetExtension(filePath);
            string zipFilePath = filePath.Substring(0, filePath.Length - extension.Length);
            System.IO.Compression.ZipFile.ExtractToDirectory(filePath, zipFilePath, true);

            //Copy Activites
            DirectoryInfo src = new DirectoryInfo(zipFilePath + "\\Activities");
            DirectoryInfo dst = new DirectoryInfo(ActivitiesDir);
            
            CopyFiles(src, dst, true, "*.*");

            //Copy FSM
            src = new DirectoryInfo(zipFilePath + "\\FSM");
            dst = new DirectoryInfo(FsmDir);

            CopyFiles(src, dst, true, "*.*");

            //Copy KPU
            src = new DirectoryInfo(zipFilePath + "\\KPU");
            dst = new DirectoryInfo(KpuDir);
            
            CopyFiles(src, dst, true, "*.*");

            //Copy Workflows
            src = new DirectoryInfo(zipFilePath + "\\Workflows");
            dst = new DirectoryInfo(WorkflowsDir);
        ...
        {           
            using (var writer = new System.IO.StreamWriter(FileName + "\\" + "Manifest.ini"))
            {
                ...
                Manifest manifest = new Manifest()
                {
                    ...
                    Files = new List<BreanosConnectors.Kpu.Communication.Common.File> { new BreanosConnectors.Kpu.Communication.Common.File { Value = "Hanoi.gif" } },
                };
        ...
        public async void PublishToServiceBus(string ZipFilePath, string connectionString, string queueString)
        {
            ...
            connector.SendAsync(packedSerialized, queueString, BrokerCommands.PACKAGE, new (string, object)[]{ ("KpuId", "Hanoi") }).Wait();
        }
    }
}
