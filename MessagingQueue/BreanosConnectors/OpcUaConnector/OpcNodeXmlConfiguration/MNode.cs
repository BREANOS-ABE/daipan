...
// <copyright file="MNode.cs" company="Breanos GmbH">
// Copyright Notice:
...
namespace BreanosConnectors
{
    ...
    {
        /// <summary>
        /// Represents a group of nodes with similar paths of the pattern [prefix]number[suffix]
        /// and the same set of children
        /// </summary>
        [XmlRoot(ElementName = "MNode")]
        public class MNode : Node
        {
            ...
            private IEnumerable<string> GetSNodeRepresenatation()
            {
                ...
                {
                    l.Add($"{PathPrefix ?? ""}{i.ToString($"D{Decimals}")}{PathSuffix ?? ""}");
                }
                return l;
            }
