...
// <copyright file="ModelUpdateLatestPropertyChangeBatcher.cs" company="Breanos GmbH">
// Copyright Notice:
...
{
    /// <summary>
    /// A specialized wrapper class for TimeBatcher and SizeBatcher
    /// designed to aggregate ModelUpdates and only forward the latest update per property
    /// </summary>
    public class ModelUpdateLatestPropertyChangeBatcher
    {
        SizeBatcher<ModelUpdate> _sizeBatcher;
        ...
        private ConcurrentDictionary<string,ModelUpdate> _batch;                              //the batch of items to send after the specified time has passed
        private Action<IEnumerable<ModelUpdate>> _collectionFinishedAction;   //callback to use for forwarding the messages
        ...
        /// </summary>
        /// <param name="mu"></param>
        public void OnMessage(ModelUpdate message)
        {
        ...
        /// <returns></returns>
        private async Task StartCollect(ModelUpdate message)
        {
            lock (_batchLock)
            {
                _batch.AddOrUpdate(message.Property, message, (key, old) => (old.TimestampUtc > message.TimestampUtc) ? (old) : (message));
