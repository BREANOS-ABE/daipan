...
namespace CoreService
{
    ...
    internal sealed class CoreService : StatelessService, ICoreService
    {
        ...
        BreanosConnectors.ActiveMqConnector.Connector _activeMqConnector, _ManagementActiveMqConnector;
        ...
        private async Task SetupActiveMq()
        {
            ...
            _ManagementActiveMqConnector = new BreanosConnectors.ActiveMqConnector.Connector();
            isConnected = await _ManagementActiveMqConnector.ConnectAsync(_amqEndpoint, _amqUser, _amqPassword);
        ...
        private void HandleRequestKPUIdRequest(string content)
        {
            var isUnpackSuccessful = BreanosConnectors.SerializationHelper.TryUnpack(content, out RequestKPUIdRequest requestKPUIdRequest);
            if (isUnpackSuccessful)
            {
                string clientId = requestKPUIdRequest.ClientId;
                var kpuIds =_currentKpuPackageRequests.Keys.ToArray();
                foreach (string key in kpuIds)
                {
                    var packedKey = BreanosConnectors.SerializationHelper.Pack(new TellKPURequest { KPUId = key});
                    _ManagementActiveMqConnector.SendAsync(packedKey, ManagementQueueString, BrokerCommands.TELLKPUID);
                }
            }
            else
                logger.Warn($"Could not unpack RequestKPUIdRequest");
        }
        ...
        public async Task RequestExecute(string id, string actionId, string[] parameters)
        {
            ...
            await _activeMqConnector.SendAsync(content, _amqRoutes["ToBlackboard"], BrokerCommands.EXECUTE_REQUEST);
        }
        #endregion
